...
| `trailing_stop_pct` | Retracement from **peak**; used only if `trailing_stop_atr_multiplier <= 0`. | Snapshot stop via `snapshot_pct_trailing_stop` in [`scanner_core/dynamic_stops.py`](../scanner_core/dynamic_stops.py) and `_compute_stop_tp_levels` when ATR mult is 0. | **Match** for level math at signal time; live still uses current price as peak (same intentional snapshot caveat as ATR trail). |
