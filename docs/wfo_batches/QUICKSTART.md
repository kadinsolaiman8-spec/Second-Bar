...
2. **Run** walk-forward the way you already do (e.g. `python -m quant.run_wfo` with your usual flags). The console will print **primary** (bootstrap) and optional **secondary** (bar permutation) **p-values**. If you use `run_wfo_batch.ps1` with the default **1000 + 500** full-history sims per ticker, expect **multi-day** wall time — see [BEGINNER_EXACT_STEPS.md — Runtime](BEGINNER_EXACT_STEPS.md#runtime-wfo-batch-wall-time).
