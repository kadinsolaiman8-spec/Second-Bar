...
def _backtest_detect_signal(indicators: dict, df: pd.DataFrame = None,
    ...
    """
    from utils import has_bullish_candle_pattern, has_bearish_candle_pattern
    from utils import check_trigger_candle_volume
...
def _run_backtest_on_df(df: pd.DataFrame, strategy_filter: str = None,
    ...
    try:
        from dynamic_stops import calculate_dynamic_stop, update_trailing_stop
        _use_dynamic = True
    except Exception:
        _use_dynamic = False
    ...
    for i in range(window_size, len(df)):
        ...
        try:
            ...
            from mean_reversion import is_mean_reversion_strategy
...
async def run_replay(ticker: str, date_str: str) -> dict:
    ...
    try:
        ...
        from indicators import run_all_indicators, count_bullish_signals
...
async def force_scan_now(tickers: list = None) -> dict:
    ...
    from scanner import scan_all_tickers, ticker_state
    from watchlist import ALL_STOCKS
...
async def run_health_check(bot=None) -> dict:
    ...
    try:
        from news import get_finnhub_client
    ...
    # 3. Scanner cache
    try:
        from scanner import ticker_state as ts, scan_history as sh
        results["cache"] = {
            "status": "PASS",
    ...
    # 4. Cooldown tracker
    try:
        from cooldown import get_system_status
    ...
    if bot is not None:
        try:
            ...
            results["discord_channels"] = {"status": "PASS", "detail": channel_report}
        except Exception as e:
            results["discord_channels"] = {"status": "FAIL", "detail": str(e)}

    # 8. Journal
    try:
        from journal import get_open_trade_count, get_win_rate
...
def generate_debug_dump(ticker: str = None) -> str:
    ...
    from scanner import ticker_state as ts, scan_history as sh, spy_cache, orb_data
    from regime import get_current_regime
    from cooldown import get_system_status
    ...
    if ticker:
        ...
        if not state:
            ...
            from cooldown import is_in_cooldown, is_muted, is_vwap_trapped
            lines.append("")
